...
### Iteration 3 — 2026-03-14 (Best-of-both synthesis)

Combined the best of iterations 1 and 2, then fixed every issue that was flagged but never resolved.

**Fixes applied (all were identified in iter 1 or 2 scoring but never completed):**

| Fix | Files Affected | Why It Matters |
|-----|---------------|----------------|
| Added `argument-hint: (no arguments)` | install.md, review.md | 30% Triggering weight; flagged in both iterations, never done |
| Fixed grep pattern `/<name>\.yaml` | pause.md | Last of 4 cron commands still using unescaped pattern |
| Replaced `/opt/homebrew/bin/tq` with `$(command -v tq)` | schedule.md, todo.md, setup-telegram.md, SKILL.md, cron-expressions.md | Portable across install locations; flagged in scoring, never applied |
| Fixed tq-message quoting (heredoc instead of single-quote assignment) | tq-message.md | Single-quote `'$SUMMARY'` won't expand; functional correctness bug |
| Refactored tq-reply slug detection (marker file + tmux fallback) | tq-reply.md | Replaced directory iteration with marker file + tmux session name extraction |
| Fixed empty REPLY_TO being passed to `--reply-to` | tq-reply.md | Omit flag entirely when no reply context exists |
| Fixed cron-expressions.md grep pattern + hardcoded path | cron-expressions.md | Manual override section had both bugs |
| Bumped SKILL.md version to 1.4.0 | SKILL.md | Reflects iteration 3 changes |

**Learnings accumulated (iteration 3):**

18. **Flagged-but-not-fixed is the gap between scoring and improvement** -- Both iterations identified these issues in their scoring output but never completed the fixes. The scoring methodology works; the follow-through loop needs to close explicitly.
19. **Consistency across file families matters more than individual file quality** -- The grep pattern was fixed in 3 of 4 cron commands. The 4th (pause.md) was missed because parallel agents didn't coordinate. Systemic fixes need a checklist, not per-file scoring.
20. **Portable paths > hardcoded paths** -- `/opt/homebrew/bin/tq` assumes Homebrew on Apple Silicon. `$(command -v tq)` works regardless of install location. This is a portability win with zero downside.
21. **Heredoc temp files are the safe quoting pattern** -- Single-quote assignments (`SUMMARY='text'`) break on content with single quotes. Heredoc to temp file handles all special characters safely — consistent with the established `load-buffer`/`paste-buffer` pattern in `anti-patterns.md`.
